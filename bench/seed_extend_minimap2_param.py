...
def override_globals(args):
    global MIN_IDENTITY, MIN_COVERAGE, MAX_EXON_DIFF, MIN_LEN_RATIO, MAX_LEN_RATIO
    if args.min_identity is not None:
        MIN_IDENTITY = args.min_identity
    if args.min_coverage is not None:
        MIN_COVERAGE = args.min_coverage
    if args.max_exon_diff is not None:
        MAX_EXON_DIFF = args.max_exon_diff
    if args.min_len_ratio is not None:
        MIN_LEN_RATIO = args.min_len_ratio
    if args.max_len_ratio is not None:
...
def main():
    ...
    ap.add_argument("--max-exon-diff", type=int, default=None)
    ap.add_argument("--min-len-ratio", type=float, default=None)
    ap.add_argument("--max-len-ratio", type=float, default=None)
    args = ap.parse_args()
