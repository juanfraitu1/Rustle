...
loci? Characterise every co-located multi-copy family on two axes:
  ...
  PSV success = fraction of reads PSV+junction confidently assigns, and the silver-standard agreement.
...
for i, (fid, dom, copies) in enumerate(fams):
    ...
    rows.append(dict(family=fid, dom=dom, n_copies=len(copies), reads=n, psv_cols=st["psv_cols"],
                     ...
                     resolv_rate=round(resolv_rate, 3), silver=round(val, 3) if val == val else None,
                     uniq=st["uniq_j"]))
...
print(f"\n{'family':10s} {'dom':9s} {'cp':>3} {'reads':>6} {'MQ0%':>5} {'PSVc':>5} {'assign%':>7} {'resolv%':>7} {'silver':>6} class")
for r in hard:
    print(f"{r['family']:10s} {str(r['dom'])[:9]:9s} {r['n_copies']:>3} {r['reads']:>6} "
          f"{100*r['mq0_frac']:>4.0f}% {r['psv_cols']:>5} {100*r['assign_rate']:>6.0f}% "
          f"{100*r['resolv_rate']:>6.0f}% {(r['silver'] or 0):>6.2f} {r['class']}")
