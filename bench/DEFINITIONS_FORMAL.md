...
> ⚠⚠ **CORRECTION 2026-08-10 — O2.** This document defined copy assignment (O2) as *"max-weight
> **facility location**"* in **three** places (the O1/O2/O4 paragraph, the "O4 is O2 turned outward"
> paragraph, and the method-objects table). `grep -rn facility --include=*.rs src/` returns **ZERO
> hits**. Facility location is an **unimplemented design goal** — a theory formulation
> (`bench/THEORY.md` §5c) machine-checked only on synthetic enumerated instances by
> `bench/mwca_approximation_check.py`, a pure-Python script that never invokes the binary. **The
> shipped O2 is a per-read decomposed argmax of the PSV+junction log-likelihood, gated by an exact
> Poisson-binomial certificate at α/(n−1).** All three sites are corrected in place below. The full
> shipped specification, with every constant at `file:line` and a 1.65 M-row reproduction test, is
> **`docs/copy_assignment_definition.md`**. *"Assign-or-abstain, no 1/k" was always accurate and is
> unchanged; only the optimization name was wrong.*
...
> **⚠ RECONCILIATION (2026-07, read with `rustle_mechanism.html`).** The POA-contiguous-core `γ_core = 0.13` predicate stated here is the *theoretical* formulation. The **shipped** family edge — in both `gw_family_catalog` and the `copy_assign`/`denovo_pipeline` path — is `refine_families_exon_sum`: an **exon-sum `asm20` alignment, identity ≥ 0.80, coverage-of-shorter ≥ 0.50, ≥ 2 distinct loci**. The `detect_families` routine that applies `core_recip ≥ 0.13` as an *edge* has **no caller in any shipped binary**; `0.13` survives live only as thin-locus **rescue** admission (`family_rescue.rs`). So when this document says `E_r iff POA-core ≥ 0.13`, the live code reads `E_r iff exon-sum asm20 id ≥ 0.80`. Both `γ_core = 0.13` and `γ = 0.20` remain named/disclosed; the *edge* number that actually runs is `0.80`, not `0.13`.
...
### Map to our objects
This **IS O1**, our primary object; its oracle is **E_r**. **E_c is not the family definition:** it was demoted to the within-family O2 structure because as a definition it silently drops divergent uniquely-mapping families (the globin problem — MB ↔ HBB share no read). **E_c ⊆ E_b** is the sole clean containment (a read conflict implies **exon-level** shared sequence; it does **not** imply full-transcript E_r homology — the 0.13 floor's ~2.3% single-conserved-exon leak is exactly the E_c ⊄ E_r gap). **χ(H) = MCC** (Lemma 1) is an O2 quantity computed inside each fixed family; copy-assignment (O2) = a **per-read decomposed argmax** of the PSV+junction log-likelihood over the family's copies, gated by an exact Poisson-binomial certificate at **α/(n−1)** (Bonferroni), **assign-or-abstain, no 1/k** — see `docs/copy_assignment_definition.md` §1. ⚠ **CORRECTED 2026-08-10: this sentence previously read "max-weight facility location". `grep -rn facility --include=*.rs src/` returns ZERO hits; facility location is an UNIMPLEMENTED DESIGN GOAL (`THEORY.md` §5c, machine-checked only on synthetic enumerated instances by `mwca_approximation_check.py`), not a description of any shipped run.** The two coincide only in the degenerate case where the open set is fixed and opening is free — which is what ships, because O1 fixes the copy set before any read is scored — and facility location has no abstention at all. **SUN** (Sudmant 2010, SUN ⊊ PSV) = the per-copy single-read identifiability witness inside the family (3-tier ladder: SUN-taggable / hap-unique-only / K-frontier). **O4** is an orthogonal axis (sample-vs-assembly) that **interacts**: multi-copy families are the most collapse-prone, so O4 recovery (hidden_copy detector: 18 collapsed flags, 4 endogenous MHC copies) lifts χ(H) toward the true genomic count.
...
### Map to our objects
**O4 is O2 turned outward:** O2 (per-read argmax + Poisson-binomial certificate at α/(n−1), assign-or-abstain, no 1/k — `docs/copy_assignment_definition.md`; ⚠ **not** "facility location", corrected 2026-08-10) assigns reads to the copies **A has**; O4 flags copies **A lacks** via the local conflict graph demanding χ(H_l) > n_loci(l). Evidence type = **PSV/SUN over-dispersion**: excess balanced columns are **SUN-consistent** witnesses of an **unenumerated** copy (SUN ⊊ PSV, Sudmant 2010 — a *genuine* SUN only once the copy is enumerated; here it is SUN-**like** evidence inferring an unenumerated hidden copy). Discipline = detect-and-flag.
...
| **Copy assignment (O2)** | which copy-path each read lies on — the **per-read argmax** of the PSV+junction log-likelihood over the family's copies, reported only when an exact **Poisson-binomial certificate** clears **α/(n−1)** and the argmax is strict; **assign-or-abstain**, **no 1/k**. Full spec: `docs/copy_assignment_definition.md`. ⚠ **This row said "max-weight facility location" until 2026-08-10; that formulation has ZERO occurrences in the Rust and is an unimplemented design goal (`THEORY.md` §5c), not the shipped object.** |
...
- **copy number** = χ_H, the **chromatic number of the read-conflict graph** = Minimum Copy Cover (Lemma 1: MCC = χ(H)). It is *not* a minimum path cover (that name belongs to the junction-augmented generalisation, CMCPC), and *not* facility location (a **theory formulation** of the O2 read→copy assignment, `THEORY.md` §5c — **never implemented**; the shipped O2 is a per-read argmax + certificate, `docs/copy_assignment_definition.md`). Keep those three names on their three objects, and keep "facility location" on the side of the line marked *design goal*.