...
- `bench/family_definition_formal` (in `bench/FAMILY_DEF.md`) — the **RNA read-conflict family**: connected component of the read-conflict graph $E_c$, refined by the significance edge $E_c^{\mathrm{sig}}\subseteq E_c$, with the copy-assignment count $\mathrm{MCC}=\chi(H)$.
...
## 5. Composition with the segdup predicate $\mathrm{SD}(\cdot)$ and relation to the RNA family $E_c$ / $\mathrm{MCC}=\chi(H)$
...
$$
E_c^{\mathrm{sig}}\subseteq E_c\subseteq E_b^{\mathrm{asym}}\qquad\text{(the only chain)};\qquad E_a\ \text{incomparable to }E_b,E_c\ \text{(4 non-containments, APOBEC3D/F witnessing }E_c\not\subseteq E_a).
$$
...
### 5.2 Relation to the RNA read-conflict family — **same skeleton, different edge oracle**

The RNA family (`FAMILY_DEF.md` §`family_definition_formal`) is the connected component of the read-conflict graph $G_c=(V,E_c)$, $|C|\ge 2$, where an edge is a read-**ambiguity** tie (de-tie / significance-tie). The DNA family is the connected component of $G_{\mathrm{DNA}}=(V,E_a)$, refined by $R$. The **skeleton is identical** (`SEGDUP_DEFINITION_FORMAL.md` §3.1: `conflict_families()` and `build` are the *same* union-find); the **only** difference is the edge oracle:

| | RNA family | DNA family (this note) |
|---|---|---|
...
$$
— RNA can resolve **at most** as many copies as the genome contains, and the **K-frontier** is exactly the strict case $\chi(H)<K_{\mathrm{DNA}}$: read-identical copies (SEDEF $\rho\to 1$, PSV density $\to 0$; RFPL4A/DAZ) that the genome distinguishes but reads cannot, forcing abstention (`SEGDUP_DEFINITION_FORMAL.md` §4.2). So $R$ furnishes the $K$ against which $\chi(H)$ is measured; the DNA family is the **ground-truth copy count**, the RNA family the **read-resolvable** subset, and their agreement region $E_a\cap E_c$ (RABL2A/B triple-core) is where both oracles see the same copies.
