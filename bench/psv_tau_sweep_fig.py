...
(2) real GGO co-located families (silver-standard agreement). Reads bench/psv_tau_sweep.json."""
...
# Panel B: real GGO — recall + silver agreement vs tau
axB = axes[1]
g = d.get("ggo")
if g:
    cur = g["curve"]; t = [r["tau"] for r in cur]
    axB.plot(t, [r["recall"] for r in cur], "-o", color="teal", label="recall (assigned/all)", lw=2)
    axB.plot(t, [r["silver_agreement"] if r["silver_agreement"] is not None and not (isinstance(r["silver_agreement"],float) and np.isnan(r["silver_agreement"])) else np.nan for r in cur],
             "-s", color="crimson", label="silver-standard agreement", lw=2)
    axB.axvline(2.0, color="gray", ls=":", alpha=.6); axB.axvline(6.9, color="purple", ls=":", alpha=.6)
    axB.set_title(f"real GGO families (n={g['n_reads']} reads / {g['n_families']} fam)\nsilver agreement vs recall as τ rises")
...
if g:
    ...
    ys = [r["silver_agreement"] for r in cur]
    axC.plot(xs, ys, "-o", color="purple")
    for r in cur:
        if r["tau"] in (0.0, 2.0, 6.9, 12.0):
            axC.annotate(f"τ={r['tau']}", (r["recall"], r["silver_agreement"]),
                         fontsize=8, xytext=(4, 4), textcoords="offset points")
    axC.set_xlabel("recall (fraction assigned)")
    axC.set_ylabel("silver-standard agreement (precision proxy)")
