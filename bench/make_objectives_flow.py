...
def build():
    ...
    stage(ax, XA, 0.35, WA, 2.35, "READS pile beyond the ref", NAVY)
    pileup(ax, XA + 2.1, 0.8, 7, "#666", w=1.4)
    ax.text(XA + 2.1, 0.55, "MAPQ 0 · depth > reference copies", ha="center", fontsize=8.2, color="#555")
    ax.text(XA + 2.1, 2.25, "collapsed / expanded in the assembly", ha="center", fontsize=8.2, color="#555", style="italic")
    ...
    ax.text(XB + WB / 2, 2.15, "MORE copy-paths than reference loci", ha="center", fontsize=8, color="#555")
    ax.text(XB + WB / 2, 0.5, "bimodal PSVs = a hidden copy", ha="center", fontsize=8.2, style="italic", color="#555")
    arrow(ax, XB + WB, XC, 1.5, "count vs reference")
    stage(ax, XC, 0.35, WC, 2.35, "REFERENCE-ABSENT COPY", RED)
    ax.text(XC + 0.35, 2.15, "reads resolve MORE copies than the", fontsize=10, weight="bold", color=DARK)
    ax.text(XC + 0.35, 1.82, "reference annotates  (χ_H or depth > ref)", fontsize=10, weight="bold", color=DARK)
    ax.text(XC + 0.35, 1.32, "cross-species check — human MAGEA:", fontsize=9.4, weight="bold", color=RED)
    ax.text(XC + 0.35, 1.00, "reads resolve 11 copies vs the 2-copy", fontsize=9.2, color="#333")
    ax.text(XC + 0.35, 0.74, "ape baseline  (matches T2T annotation)", fontsize=9.2, color="#333")
    ax.text(XC + WC / 2, 0.42, "detect + flag (copy-vs-allele needs DNA)", ha="center", fontsize=8.2, style="italic", color="#555")

    fig.suptitle("From reads to results — the four objectives on real Iso-Seq data",
