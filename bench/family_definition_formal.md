...
| **$E_r$ precision / FP split** (headline) | `bench/family_er_pr.py` → `family_er_pr.{tsv,json}`, `FAMILY_ER_PRECISION_RECALL.md` (refined $E_r$ projected to genes by min-overlap-floored max-overlap; FP split by protein truth $E_p$ = reciprocal whole-protein mmseqs, no id floor) | refined $E_r$ **10,755 gene-pairs**; genuine precision **0.613** (truth-bar-misses counted correct); the **6,142** FP with a reciprocal-protein edge (**99.3% tier-1**, prot-id median 47%) are **real divergent paralogs the cDNA-90% truth misses, not errors**; **GENUINE over-merge = 4,163 edges (38.7%)** — headline the **family-level 115/858 = 13.4%** Ep-impure blocks, not the clique-inflated edge rate; min-overlap floor removed 46 spurious $<$20%-clip attributions (pre-floor upper bound 4,209). **No circular "effective $\approx$1.00"** |
| $E_r$ recall vs $E_c$ (identity-stratified) | `bench/family_er_pr.py` (recall vs cDNA-90% truth `family_def_dna_pr_edges.tsv`, subsets a/b absolute + c reachable) | **TRADE-OFF, not dominance.** ABSOLUTE recall (expressed & cross-map) $E_r$ **0.150** $<$ $E_c$ **0.552** — $E_r$'s de-novo footprint reaches only **13.7%** of truth genes. On the **reachable** subset (both loci in-footprint) $E_r$ **0.949** vs $E_c$ **0.686** (+0.26, isolates the CRITERION; the cut drops 1,782 pairs, $E_c$ loses 940 correctly-grouped TP, $E_r$ loses 0 by construction). Low-id win is the smoking gun: id 0.90–0.95 reachable $E_r$ **0.951** vs $E_c$ **0.328**. The +0.26 must ALWAYS travel with the 13.7% representation bound |
