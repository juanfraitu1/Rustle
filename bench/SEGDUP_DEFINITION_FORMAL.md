...
> **Update (2026-06-30) — the containment tightens one level, to the transcript oracle $E_r$.** `bench/family_definition_formal.md` inserts a **fourth homology oracle** between $E_b$ and $E_p$: the **spliced-transcript** oracle $E_r$ (significant transcript self-alignment; expressed loci $V_R$). Because $E_c$ and $E_r$ **share the node set $V_R$** (both are de-novo expressed loci) while $E_b$ crossed to the larger 34,114-locus genome set, the tightest clean containment moves **down one level**:
> $$E_c^{\mathrm{sig}}\subseteq E_c\subseteq \boxed{E_r^{\mathrm{asym}}}\ \subseteq_{\mathrm{proj}}\ E_b^{\mathrm{asym}},$$
> and $E_c\subseteq E_r^{\mathrm{asym}}$ is asymmetric for exactly the same reason $E_c\subseteq E_b^{\mathrm{asym}}$ is (a de-tie read need be exonic/homologous at only one endpoint; `read_conflict.rs:86–106` checks no partner-exon overlap). This **supersedes $E_c\subseteq E_b$ as the tightest containment** but does not disturb it (still valid by projection). The full **O1 lattice is now four homology oracles** $E_a$—$E_b$—$E_r$—$E_p$ (pairwise incomparable), and **$E_c$ (read-ambiguity) is demoted out of the O1 lattice** to a within-family **O2** copy-assignment substructure of $E_r$ (with the SUN 3-tier ladder). $E_r$ vs $E_b$ is a **symmetric refinement, incomparable overall** ($E_r$ = the "exonic-at-BOTH" symmetric core of $E_b$, plus a spliced-substrate superset direction that catches retrocopies $E_a/E_b$ miss). See `bench/family_definition_formal.md` §4, §6 and `bench/RNA_FAMILY_HOMOLOGY_REFRAME.md`.
