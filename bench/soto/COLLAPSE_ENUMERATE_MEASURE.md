...
**GWFAMc0 — ANKRD20A (Soto `ID_280`, 3 annotated members), famCN=4, alt_frac=0.567, 234 alt reads**
`seed chr9:40,211,594-40,234,709` → `ANKRD20A3P` (d=0). Projection loci:
...
**GWFAMc1 — chr1q21 segdup (Soto `ID_211`, 4 annotated members), famCN=3, alt_frac=0.400, 12 alt reads**
`seed chr1:148,825,999-148,829,853` → `AC243772.3` (d=0). Projection loci:
...
projection locus lands on a real annotated member of that same family (`ID_211`: 4/4) — except one
`ID_280` locus that is a bona-fide *unannotated* ANKRD20 copy (famCN=4 correctly exceeds the 3
...
default (OFF) path is unchanged and unaffected.
