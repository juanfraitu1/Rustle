...
- TBC1D3 (9 protein-coding copies): level 1 = genomic cluster 1 (proximal 37.1-37.44 Mb: B,I,G,H,F) vs cluster 2
  ...
  B, F, G, H, I singletons. [superseded by the TBC1D3 truth addendum]
...
NPIPB subfamily" {B3, B4, B5, B11, B12, B13}. TBC1D3: AE {TBC1D3, E} and CDKL {D, K}; cluster1|cluster2 is POSITIONAL. [superseded by the TBC1D3 truth addendum]
...
-alrt 1000 -T 4 --seed 1`; supported = SH-aLRT > 75. Groups: NPIP (P1's six), TBC1D3 (AE, CDKL, positional check), [superseded by the TBC1D3 truth addendum]
...
positional, AMY AMY1/AMY2/AMY2Ap) [superseded by the TBC1D3 truth addendum]. Descriptive; expectation stated now: DN1 lowers NPIP families per record without
raising collapse.
...
**AP status update (2026-09-14, after AP-1 SUPPORTED): promoted to the shipped default.** User decision: make
`min_shared_exon_frac` default-on rather than opt-in. Implemented as a CLI-level default flip only
(`mcl_families --min-shared-exon-frac`'s `default_value_t` 0.0 -> 0.30); the `GraphParams` struct's own `Default` impl
stays 0.0 (library/test-conservative), mirroring exactly how `--exonic-both-sides` was flipped in §6ey. Verified: the
freshly built release binary run with NO flag reproduces the explicit `--min-shared-exon-frac 0.30` catalog on
chr5/7/21 byte-for-byte; `cargo test --lib` unaffected (852 passed before and after, the flip touches no other test).
`--min-shared-exon-frac 0.0` reproduces every catalog built before 2026-09-14.
⚠ **No existing on-disk catalog is retroactively changed** — every guided-truth artifact used so far this session
(gorilla `gw_units_v3`, `human2/guided.clusters.tsv`, and every `e1`-tagged catalog built today before this flip) was
generated by an explicit `mcl_families` invocation that predates the flip and did not pass the flag, so it reflects
0.0, not 0.30. Regenerating gorilla's production truth (SEDEF + core-refine, hours) or human's is a separate,
explicitly-requested follow-up, not done here.

---
## ADDENDUM AQ (2026-09-16, post-hoc TBC1D3 level-2 truth amendment) — the name-mapped TBC1D3 subfamily truth was wrong for 7/9 CHM13 copies

**Why:** the TBC1D3 level-2 truth used throughout this document (Addenda G, and every place tagged
"[superseded by the TBC1D3 truth addendum]" above: lines ~223-225, 579, 671-672, 772) was a **name-based
guess** (AE = {TBC1D3, TBC1D3E}, CDKL = {TBC1D3D, TBC1D3K}), explicitly flagged there as a "declared
assumption" because "the paper's per-copy CHM13 table is not on disk". `bench/TBC1D3_GUITART_TRUTH_CORRECTION.md`
(2026-09-16) re-derives the per-copy truth from Guitart et al. 2024 Fig 6B/6C (vector-geometry arrow-to-copy
matching, cross-checked against CHM13/GRCh38 sequence distances and population trees) and finds the old map
wrong for 7 of 9 copies.

**Corrected truth, three variants (full detail: `bench/TBC1D3_GUITART_TRUTH_CORRECTION.md` §0, §1):**
- **F** (figure, all 9 copies): M = {TBC1D3B, TBC1D3H}; CDKL = {TBC1D3K, TBC1D3}; TBC1D3D → AE (figure-only);
  TBC1D3I, TBC1D3F (labelled G), TBC1D3G (labelled B), TBC1D3E (labelled O) singletons.
- **S** (F with TBC1D3D's level-2 excluded — not "sequence-safe", it still keeps K in CDKL).
- **V3** (F with TBC1D3D moved into CDKL).
- Robust across all three: **M = {TBC1D3B, TBC1D3H}** (figure, sequence and population trees agree) and
  **TBC1D3 (unsuffixed) in CDKL** (also has a textual anchor: Guitart p.15-16, the expressed CDKL paralog is
  the last, telomeric-facing copy of cluster 2 in 67/69 haplotypes). **Moderate:** TBC1D3K in CDKL (its 6B
  arrow is hand-placed). **Unresolved:** TBC1D3D's group (figure says AE, sequence leans CDKL).

**No registered reading in this document changes verdict:**
- **AA (subfamilies from split-dominance bubbles) is NOT SUPPORTED under every variant**: IQ-TREE vs DT
  "either"-class recoveries are 100 vs 84 (F), 97 vs 84 (S), 94 vs 84 (V3) — all still far apart, same verdict
  as the original 85 vs 73.
- **B2 still excludes TBC1D3** from the coding-vs-noncoding clade comparison (unaffected by which copy holds
  which label).
- **O-4 and P-3** (the positional cluster1|cluster2 readings, wherever referenced above) are unaffected: the
  positional call uses genomic coordinates only, never the per-copy phylogenetic label.

**Historical-record note:** the old name-mapped truth text at the tagged lines is left in place (append-only
discipline); this addendum is the correction of record. See `bench/TBC1D3_GUITART_TRUTH_CORRECTION.md` §6 for
the full, line-by-line list of downstream documents/code also corrected on 2026-09-16.
