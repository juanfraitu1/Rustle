...
| **12** | ⭐⭐**A read that SPLICES OVER a locus is not evidence for it — `samtools view -c REGION` counts the wrong thing.** | 2026-09-02 (§6cm): inflated a headline **3.4×**. `NPIPP1` looked like 1,608 reads collapsing to a 4-read locus; **1,151 = 71.6%** merely spliced across, because `PDXDC1` (168 kb) CONTAINS it. Real figure **457**, itself an upper bound. The rule was already project doctrine and enforced nowhere. **Use `bench/locus_reads.py::reads_with_block_in`.** |
| **13** | ⭐⭐**Before blaming the pipeline for an oversized locus, ask what gene SPANS it.** | 2026-09-02 (§6cm): two "mis-chained giants" were `SNX29` (covers **100.0%**) and `PDXDC1` (**99.9%**) — real genes, correctly assembled, with canonical junctions and majority read support *because they are real transcripts*. A rule fitted to that 3-node "blob class" would have been fitted to two real genes. **`bench/locus_reads.py::spanning_genes`.** |
