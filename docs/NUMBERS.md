...
| **3/150 = 2.00%** [0.0068, 0.0571] false-merge at the SHIPPED node floor | rate, same panel, **floor 2** | **HUMAN** CHM13 v2.0 / `A119b.t2t.bam`, 150 single-locus windows | ⚠⚠**Quote the node floor with the rate.** Floor 3 gives 1.33% on a **disjoint** window set (W063/W106 → W033/W034/W065). One parameter, sets swap completely ⟹ §6ac's 3→2 is the whole cause; **6/6 floor-2 edges carry a 2-read endpoint**. 2 vs 3 events — **not** a significant difference. §6bt.1 |
| **5/5** pre-registered criteria, **9/9** edge outcomes | cross-species test of `RUSTLE_ER_COVERAGE_LONGER_FLOOR` | clause developed on **GORILLA** NPIP; panel is **HUMAN**, frozen 08-10 | Prediction committed as `5cbced4` and pushed **before** the run; mechanistic, not a count. ⚠**Do NOT say "the rate halved"** — 2–3 events, intervals overlap. §6bq/§6bt.2 |
| **7.09%** (GGO) / **7.10%** (PTR) families with a self-overlapping copy | rate, pure coordinates | the `refine`-built genome-wide catalogs — ⚠**not reproducible** | Cross-species replication at nearly the same value ⟹ a **pipeline** property. Current 3-contig path: **0/4,176 pairs** [0, 0.0009]. ⛔**Not "fixed genome-wide"** — substrates unmatched. §6bs |
| **776/788 = 98.5%** of annotation agreements are one string | share, external-corroboration attempt | **GORILLA** `GGO_genomic.gff` vs `arm_f2` edges | `"zinc finger protein"`, carried by **552 genes**. Strip ZNF: 12/34. Non-generic target band **0/8**. ⟹ the annotation **cannot adjudicate** the 0.80–0.90 band — which is **not** the same as the edges being wrong. §6br |
| **220 lost / 32 gained** copies, **29 at loci with none** | catalog delta at `cov_longer` ≥ 0.30 | **GORILLA** `arm_cfoff` → `arm_cf030`, 3 contigs | ⚠⚠**The clause is NOT monotone at the catalog level** (node set identical, 3,598, 0 diff). Copy loss is **annotation-neutral** within exon strata (single-exon 0.013 vs 0.009; multi-exon 0.791 vs 0.784). **18 families deleted outright.** Verdict **KEEP OPT-IN**. §6bv |
| **54 → 32** largest family | family-structure precision, undisputed | same arms as above | The ZNF blob splits (consistent with §6bi). ⭐**Quotable independently of the copy-level reasoning.** 2-member families 40 → 48. §6bv |
