...
**Goal:** Copy-assignment as the maximum-likelihood soft relaxation of **SDA's PSV correlation-clustering** (Vollger 2019) on the PSV-aware VG: E-step = the existing per-read PSV/junction threading likelihood (SDA attraction/repulsion made continuous), M-step = copy-path abundance, iterated to convergence, emitting a per-read soft posterior labeled by the `min_p` identifiability bound — plus the SDA-derivation + consistency theorem (Strong-Separation ⟹ unique path-cover) and a coverage sweep that shows SDA's 91–93% climbing to 100% as coverage grows.

**Architecture:** The EM is the maximum-likelihood **soft relaxation of SDA's PSV correlation-clustering** (Vollger 2019, the paper the advisor sent) run on the thesis's existing **PSV-aware VG** (copies = read-supported PSV-bubble paths, PSVs = bubbles, assignment = threading). A new `em_copy_assign.rs` module of pure EM functions consumes the per-read per-copy threading log-likelihood extracted (DRY) from `copy_assign::assign_read_editing` — that likelihood IS SDA's attraction/repulsion made continuous. A `--em` mode in the `copy_assign` binary builds the evidence from a family's reads/copy-paths (the VG/gate integration), runs the EM, emits two TSVs. Identifiability = the existing MCC=χ(H)/Strong-Separation theorem; validation = a planted-ground-truth coverage sweep (SDA 91–93% → 100%) + the derivation write-up. Additive: default output unchanged.
...
Doc deliverable (no code). This is the **SDA-derivation** — the part that makes the method believable, not invented. Contents: (1) **Derivation from SDA** — restate SDA's PSV graph (attraction = a read carrying two PSVs → same copy; repulsion = mutually-exclusive), and show the EM's per-read likelihood `L_rk` IS SDA's attraction/repulsion made continuous (a read carrying copy `k`'s private allele at a bubble adds `log(1−e)` to `k`, `log(e/3)` to the rest); the E-step is SDA's read-partition made soft, the M-step estimates path usage. Cite `reference_sda_vollger` explicitly. (2) the mixture-over-copy-paths model + assumptions; (3) **Theorem** — in the identifiable regime (every copy pair separated by a bubble where `min_p < alpha/(K-1)` is achievable = Strong-Separation) the mixture is identifiable and the EM/MLE is consistent (`π̂→π*`, MAP `ẑ_r→z*_r` as coverage `n→∞`); (4) proof sketch: finite-mixture identifiability + MLE consistency (Redner–Walker 1984) specialized to the discrete PSV emission, with the identifiability partition = the `min_p` certificate = SDA's separability; (5) tie to `bench/copy_assignment_theory.md`: this regime is exactly its **Strong-Separation ⟹ unique minimum path-cover (MCC = χ(H))** result — EM converges to that cover; K=0 = SDA's ">100 kb reads" floor, a posterior-invariant class surfaced as `SoftZone`; (6) the theorem **explains SDA's empirical 91–93% accuracy floor** and predicts the Task 6 coverage sweep. Reference IsoCon (PSV-vs-error) and Sudmant SUN as the per-column filters feeding the bubbles.
