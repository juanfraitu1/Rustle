...
| **L4** | **subfamily (Iso-Seq group), applied SELECTIVELY** | t_4 = t_3 ∧ w_98 ≥ **0.995**, and only where the L3 group GENUINELY SUBDIVIDES (≥ 2 parts of ≥ 2 nodes) | ⭐**NEW 2026-09-19 (§6p5).** One cut cannot serve two granularities: at 0.985 the Dishuck **Iso-Seq groups** score F 0.667, at 0.995 the coarser subfamily view drops to 0.683. Held out on TBC1D3's Guitart truth the tighter cut takes specificity **0.000 → 0.750 at no recall cost** while collapsing recall on NPIP's coarser view (0.503 → 0.105). **Selective L4 is the best of all arms — lit 0.714 / iso 0.833, mean 0.774** (vs L3-only 0.720, L4-everywhere 0.758) and touches only **4 of 38 groups**; the coarse view keeps bipartite P 1.000 and pairwise precision 1.000. **Nesting is FREE**: 0.995 ≥ L3's 0.985 makes t_4 subgraph-monotone, so T1/T1′ hold by the existing proofs — verified 30/30 L4 groups inside one L3 group, 0 violations. `engine.l4_refine(aggs, l3_groups, selective=True)`; labels are prefixed `L3:`/`L4:` so the level used is always visible. ⚠Calibrated on NPIP (development) with a 2-pair held-out check on TBC1D3 |
