...
Generated 2026-09-19 at `dna-from-genome@7b18565c` by `tools/audit_cleanup_candidates.py` (re-run it; this file is overwritten). **Read-only: nothing was moved, edited or deleted.** Full per-file table: `docs/cleanup_candidates.tsv` (filter on `class` and `confidence`).
...
| **SUPERSEDED-VERSION** | medium | Older member of a version series in the same directory (_v1.._vN, foo/foo2/foo3, dated _YYYY-MM-DD copies, foo vs foo_fix/_final/_new) that nothing anchors. 'high' when the newest member IS anchored. | 13 | 95K |
...
| **AMBIGUOUS-CITE** | low | Not anchored; an anchor source names it only by a bare basename shared by several files, a directory too large (>40 files) or a glob matching several files -- may or may not mean this copy. | 621 | 32M |
| **ORPHAN** | medium | Not anchored; named only by files that are themselves not anchored (e.g. a figure named only by its un-cited plotting script). | 252 | 38M |
...
| **PROBABLE-PROVENANCE** | - | Not cited by name, but it sits in an experiment directory (below bench/, docs/, ...) holding anchored files, or under a folder whose anchored README/write-up covers it -- usually an output of that experiment written under a computed name and cited as a folder. Verification judged 5/6 such files provenance: not a candidate. | 105 | 288K |
| **UNCITED-RECENT** | low | Not cited, touched within 30 days: may be work in progress. | 29 | 1M |
| **KEEP-CITED** | - | Anchored directly or transitively. Not a candidate. | 1223 | 222M |

**1395 candidates** of 2880 files (0 high, 661 medium, 734 low confidence).
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| `bench/soto/` |  |  |  |  |  |  | 285 |  |  |  | 95 |
| `bench/` |  |  |  |  |  |  | 12 | 101 | 153 |  | 563 |
| `archive/bench/` |  |  | 23 | 9 | 2 | 16 | 188 | 1 | 14 | 5 | 221 |
| `analysis/family_graphs/` |  |  |  |  |  |  | 24 | 72 | 46 |  | 62 |
| `bench/psv_split_stringtie/` |  |  |  |  |  |  |  |  | 80 |  | 0 |
| `bench/graph_align_experiment_structural/` |  |  |  |  |  |  | 48 |  |  |  | 0 |
...
| `bench/identifiability_boundary/` |  |  |  |  |  |  |  | 5 |  |  | 0 |
...
- `archive/bench/family_er_pr.py` [tracked, low] → superseded by `src/rustle/vg_family/mod.rs` (partial-port;still-used-by:archive/bench/exon_containment_criterion.py;match:glob-wide+import+suffix)
...
- `archive/bench/family_rna_refine.py` [tracked, low] → superseded by `src/bin/family_define.rs,src/rustle/vg_family/driver.rs,src/rustle/vg_family/mod.rs` (still-used-by:archive/bench/family_merge_colinear.py;match:glob-wide+import+suffix)
- `archive/bench/genome_family_def.py` [tracked, low] → superseded by `src/rustle/vg_family/mod.rs` (still-used-by:archive/bench/colinear_multiexon_gate.py;match:glob-wide+import+suffix)
...
- `archive/bench/o2_vg_visualization.py` [tracked, low] → superseded by `src/rustle/vg_family/mod.rs,src/rustle/vg_family/o2_materialize.rs` (partial-port;still-used-by:archive/bench/known_family_showcase.py;match:glob-wide+import+suffix)
...
- `archive/bench/poa_family_definition.py` [tracked, low] → superseded by `src/rustle/vg_family/family_graph.rs` (still-used-by:archive/bench/candidate_generation_recall.py;match:glob-wide+import+suffix)
- `archive/bench/psv_graph_genomewide.py` [tracked, low] → superseded by `src/rustle/vg_family/mod.rs,src/rustle/vg_family/o2_columns.rs` (partial-port;still-used-by:archive/bench/a1_read_sda_smoketest.py;named-beside-stale-language-by:memory:project_psv_aware_vg.md;match:glob-wide+import+suffix)
...
- `archive/bench/recombination_bridge_detector.py` [tracked, low] → superseded by `src/rustle/vg_family/bridge_detector.rs,src/rustle/vg_family/mod.rs` (still-used-by:archive/bench/colinear_multiexon_gate.py;match:glob-wide+import+suffix)
- `archive/bench/rna_only_edge_oracle.py` [tracked, low] → superseded by `src/rustle/vg_family/mod.rs` (still-used-by:archive/bench/colinear_multiexon_gate.py;match:glob-wide+import+suffix)
...
- `archive/bench/family_def_refute_edgebetw.py` [tracked, medium] → superseded by `archive/bench/family_def_refute_edgebetw3.py` (series:n;match:glob-wide)
- `archive/bench/family_def_refute_edgebetw2.py` [tracked, medium] → superseded by `archive/bench/family_def_refute_edgebetw3.py` (series:n;match:glob-wide)
- `archive/bench/gw_rebuild_v3.sh` [tracked, medium] → superseded by `archive/bench/gw_rebuild_v4.sh` (series:v;match:glob-wide)
- `archive/bench/refute_cov_min.py` [tracked, medium] → superseded by `archive/bench/refute_cov_min2.py` (series:n;match:glob-wide)
- `archive/bench/soto/member_attribution.py` [tracked, medium] → superseded by `archive/bench/soto/member_attribution_final.py` (series:fix;match:dir-large)
