...
Generated 2026-09-22 at `dna-from-genome@f6b620de` by `tools/audit_cleanup_candidates.py` (re-run it; this file is overwritten). **Read-only: nothing was moved, edited or deleted.** Full per-file table: `docs/cleanup_candidates.tsv` (filter on `class` and `confidence`).
...
|---|---|---|---:|---:|
| **PROTECTED** | - | build/config files, Rust sources and tests (Rust reachability is docs/MODULE_STATUS.md's job, enforced by module_status_tests), and the anchor docs themselves. Never a candidate. | 186 | 7M |
...
| **SUPERSEDED-PORTED** | medium | Python script that a Rust source line declares it ports ('Port of', 'Faithful Rust port of', 'Mirrors', 'migration'); 'low' when only a function or part is ported (`x.py::f`, `x.py loaders`). The Python may still serve as a parity oracle or golden-fixture generator -- check tests before deleting. | 1 | 2K |
...
| **AMBIGUOUS-CITE** | low | Not anchored; an anchor source names it only by a bare basename shared by several files, a directory too large (>40 files) or a glob matching several files -- may or may not mean this copy. | 12 | 184K |
...
| **KEEP-CITED** | - | Anchored directly or transitively. Not a candidate. | 465 | 22M |

**34 candidates** of 688 files (1 high, 4 medium, 29 low confidence).
...
- `bench/mcl_port.py` [tracked, low] → superseded by `src/bin/mcl_port.rs` (still-used-by:bench/adjudicated_truth.py;match:exact+glob-wide+import+suffix)
