...
# Output: mcl_ann/gw_units_v3.*  Then: bench/o2_sweep_split.py gw_units_v3 sweep_gw_v2 and sweep_gw_v2/run_sweep.sh.
set -u
...
cd $M && ( ulimit -v 20000000; /usr/bin/time -f "TIME %e s rss %M kB" $MF --paf allgenes_gw.asm20.paf --gff $W/GGO_genomic.gff --min-exonic-bp 1 \
  ...
  --bam $W/GGO_ds.bam --fasta /home/juanfra/winloci_scratch/GGO.fasta --out $M/gw_units_v3 ) > $M/adj/gw_units_v3.log 2>&1
echo "gw exit=$?"; tail -3 $M/adj/gw_units_v3.log
