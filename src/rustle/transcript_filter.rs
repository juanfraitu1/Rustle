...
pub fn pairwise_overlap_filter_with_summary(
    ...
    // RUSTLE_RI_LOCAL (default OFF): supplementary retained-intron pass.
    //
    // The main pairwise loop only tests a victim n2 against killers n1 that are
    // in n2's significant-overlap adjacency (`ord_neighbors`) AND come earlier in
    // `ord` (`ord_pos[n2] > oi`). ST's pairwise retained_intron, by contrast,
    // tests each prediction against every LOCAL higher-coverage overlapping
    // transcript. Under the flag we widen the retained_intron kill ONLY: for each
    // still-live, non-guide victim n2, test `retainedintron_like` against every
    // higher-`tx_score` (earlier-`ord`) overlapping killer n1 — regardless of the
    // significant-overlap adjacency / ord restriction. Everything else (other
    // pairwise reasons, the lowintron flags, the kill bookkeeping) is reused
    // unchanged so default behavior is byte-for-byte identical when the flag is OFF.
    if crate::stringtie_parity::st_ri_local() && lowintron.is_some() {
        let li = lowintron.as_ref().unwrap();
        for oi in 0..ord.len() {
            let n2 = ord[oi];
            if dead.contains(n2) {
                continue;
            ...
            let t2 = &txs[n2];
            // Mirror the main-loop RI gate: victim must be non-guide.
            if is_guide_pair(t2) {
                continue;
            }
            let Some((s2, e2)) = tx_span(t2) else {
            ...
            // Killers are strictly higher-score => earlier in `ord` (oj < oi).
            for oj in 0..oi {
                let n1 = ord[oj];
                if dead.contains(n1) {
                    continue;
                }
                // Skip pairs the main loop already tested (n1 is a sig-overlap
                // neighbour of n2): those went through the identical RI check.
                if overlap_sets[n2].contains(&n1) {
                    continue;
                }
                let t1 = &txs[n1];
                let Some((s1, e1)) = tx_span(t1) else {
                ...
                // Local killer: spans must overlap.
                if e1 < s2 || e2 < s1 {
                    continue;
                }
                if retainedintron_like(&txs, n1, n2, li, error_perc) {
                    if crate::parity::decisions::is_enabled() {
                        let ktx = &txs[n1];
                        let vtx = &txs[n2];
                        let pk_p = format!(
                    ...
                    // Inline kill bookkeeping mirroring the main-loop `kill!` macro
                    // (scoped to retained_intron only). Keeps parity emit + summary
                    // counters consistent with the main pairwise pass.
                    dead.insert_grow(n2);
                    let vtx = &txs[n2];
                    if should_trace_transcript(vtx) {
                        log_filter_decision(vtx, "pairwise_overlap_filter", false);
                        let killer_tx = &txs[n1];
                        let vtx_id = get_tx_display_id(vtx);
                        let killer_id = get_tx_display_id(killer_tx);
                        eprintln!("[FILTER-DETAIL] {} @ pairwise_overlap_filter: killed_by {} reason=retained_intron_local",
                                  vtx_id, killer_id);
                    }
                    if tx_in_trace_locus(vtx) {
                        let extra = format!("KILLED_BY={} REASON=retained_intron_local", n1);
                        trace_tx_detail("PAIRWISE_KILL", vtx, Some(&extra));
                    }
                    if predcluster_trace {
                        let vstart = vtx.exons.first().map(|(s, _)| *s).unwrap_or(0);
                        ...
                        let kend = ktx.exons.last().map(|(_, e)| *e).unwrap_or(0);
                        eprintln!(
                            "--- print_predcluster: FILTER pred[{}] {}-{} cov={:.4} strand={} exons={} KILLED_BY pred[{}] {}-{} cov={:.4} reason=RETAINED_INTRON_LOCAL",
                            n2, vstart, vend, vtx.coverage, vtx.strand, vtx.exons.len(),
                            n1, kstart, kend, ktx.coverage
                        );
                    }
                    if debug_detail {
                        kill_reason[n2] = "retained_intron";
                        kill_by[n2] = n1;
                    }
                    classify_pairwise_reason("retained_intron", &mut pairwise_summary);
                    *reason_counts.entry("retained_intron").or_insert(0) += 1;
                    if crate::parity::decisions::is_enabled() {
                        let vtx = &txs[n2];
                        let pk_s = vtx.exons.first().map(|(s, _)| *s + 1).unwrap_or(0);
                        let pk_e = vtx.exons.last().map(|(_, e)| *e).unwrap_or(0);
                        let ktx = &txs[n1];
                        let pk_p = format!(
                            r#""reason":"retained_intron","cov":{:.4},"nexons":{},"stage":"pairwise","killer_cov":{:.4},"killer_nexons":{},"killer_start":{},"killer_end":{}"#,
                            vtx.coverage, vtx.exons.len(),
                            ktx.coverage, ktx.exons.len(),
                            ktx.exons.first().map(|(s, _)| *s + 1).unwrap_or(0),
                            ktx.exons.last().map(|(_, e)| *e).unwrap_or(0)
                        );
                        crate::parity::decisions::emit(
                    ...
                    break; // victim n2 is dead; move to next victim
                }
            }
        }
    }
